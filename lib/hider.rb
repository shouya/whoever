...
class Hider
  ...
  def query_real_value(table, fake_value)
    @table[table] = Hash.new(:r2f => {}, :f2r => {}) unless @table.key? table
    if @table[table][:r2f].key? fake_value
      return @table[table][:r2f][fake_value]
    else
  ...
  def query_fake_value(table, real_value)
    ...
    if @table[table][:f2r].key? real_value
      return @table[table][:f2r][real_value]
    end
