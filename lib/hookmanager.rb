...
class Whoever::HookManager
  ...
  def set_hooks
    ...
    register_hook(:GET, %r{/statuses/show/.*\.json}, ::TimelineHook)

    register_hook(:POST, %r{/statuses/update\.json}, ::StatusUpdateHook)

    register_hook(:GET, %r{/friendships/show\.json.*}, ::FriendshipShowHook)
  end
end
